...
raw_data_dir <- "/scratch/project_2007415/DATA2023/Pisa2023/"
old_data_dir <- "/projappl/project_2003061/repos/microclim_pisa/output/"
...
# el <- read_csv("/projappl/project_2003061/repos/microclim_pisa/output/error_log.csv") %>%
...
el %>% filter(site == "PIS120")
# List new error dates and codes, see the codes above
nel <- bind_rows(
  tibble(site = "PIS025", tomst_id = 94194268, start_date = as_date("2022-10-07"), end_date = as_date("2023-08-08"), probl = 3)
)
...
# Site PIS100 - PIS120 have rubbish moisture data until 2022-10-06
# I will just change these to NA

df <- df %>% 
  mutate(moist = ifelse((site %in% paste0("PIS",100:120) & as_date(datetime) < "2022-10-07"), NA, moist))
...
# Some weird timeshift in PIS047 in 2022−06−27
# For now i will delete data after that. If happens next year as well could be looked into

df <- df %>% 
  filter(!(site == "PIS047" & as_date(datetime) > "2022-06-26"))
