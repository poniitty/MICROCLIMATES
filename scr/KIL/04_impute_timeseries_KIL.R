...
df <- df %>% 
  filter(!startsWith(site, "SAL"))%>% 
  filter(!startsWith(site, "SE"))%>% 
...
dfw <- dfw %>% 
  rowwise() %>%
  mutate(NA_prop = sum(!is.na(across(4:ncol(.))))) %>% 
  filter(NA_prop > 20) %>% 
  select(-NA_prop) %>% 
  ungroup()
...
df_all <- df_all %>% filter(!is.na(T3))

df_all %>% fwrite(paste0("output/",area,"/tomst_data_imputed.csv"))
