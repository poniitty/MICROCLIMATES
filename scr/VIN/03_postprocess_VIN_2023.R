...
# df %>% filter(T1h > 2 & T1l > 2)
# df %>% filter(site == "VIN032" & datetime > as_date("2022-08-30")) %>% as.data.frame()
...
df <- df %>% 
  ...
  mutate(outlier = ifelse(abs(moist_diff - moist_diff_roll) > 1000, TRUE, outlier),
         ...
         outlier = ifelse(moisth > 500 & moistl > 500, TRUE, outlier),
         # Sudden reversible peak is higher than 300
         outlier = ifelse(moisth < (-200) & moistl < (-200), TRUE, outlier)
